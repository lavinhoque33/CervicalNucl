...
file = open("TrainingVals.txt", "a+")
for ranges in range(30, 50, 5):
    for min_size in range(5, 6):
        for max_size in range(50, 155, 10):
            for wind in range(35, 57, 2):
                for conc in range(38, 57, 2):
                    for inert in range(20, 50, 10):
                        for solidit in range(60, 81, 10):
                            iner = float(inert) / 100.0
                            solid = float(solidit) / 100.0
                            ...
                            total_aji = 0
                            ind = 0
                            basepath = 'D:\Downloads\Compressed\segmentedImg\segmentef'
                            for f in listdir(basepath + '/Train'):
                                ind += 1
                                img = cv.imread(os.path.join(basepath, 'Train', f))
                                fin2 = cv.imread(os.path.join(basepath, 'Train Mask', f.split('.')[0] + '_Mask.jpg'))
                                img1 = np.copy(img)
                                img = img[:, :, 1]
                                img2 = img
                                fin2 = fin2[:, :, 0]
                                ...
                                th2 = cv.adaptiveThreshold(img, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY_INV,
                                                           wind, conc)
                                th3 = th2
                                ...
                                (final, nuclD) = recoverNucl(img2, fin1, min_size, max_size, solid, iner, ranges)
                            ...
                            if(avg_precision > bscore):
                                bscore = avg_precision
                                branges = ranges
                                bsolidit = solid
                                bwind = wind
                                bconc = conc
                                ...
                                bmin_size = min_size
                                bmax_size = max_size
                                print('Best Score Change: ', bscore)
        print('new ranges******************************************************')
...
print("Values: ", branges,bsolidit,bwind,bconc,binert,bmin_size,bmax_size)
